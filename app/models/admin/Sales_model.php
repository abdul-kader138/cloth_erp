...
//                        $this->UpdateCostingAndPurchaseItem($return_item, $return_item['product_id'], $return_item['quantity']);
    ...
    public function getAllPaymentsFroPosCustomer($customer_id)
    {
        $this->db->select('sum(p.pos_paid) as pay_val')
...
//            ->where('type', 'received')
    ...
    public function getAllSalesDetails($ref)
    {
        $this->db->select("sales.*,companies.vat_no");
        $this->db->from('sales');
    ...
    public function getTodayCreditSalesReport($start_date,$end_date)
    {
        $this->db->select('SUM( COALESCE(grand_total, 0 ) ) AS total, SUM( COALESCE( pos_paid, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
//            ->where('type', 'received')
            ->where('payments.payment_method', 'credit')
            ->where($this->db->dbprefix('payments') . '.date BETWEEN "' . $start_date . '" and "' . $end_date . '"');
