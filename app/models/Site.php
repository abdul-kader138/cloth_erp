...
class Site extends CI_Model
{
    ...
    public function checkTaxInfoPROOF($biller, $type)
    {
        ...
        if (!isset($biller->sequence_limit_proof) || !isset($biller->expire_date_proof) || !isset($biller->sequence_proof) || !isset($biller->prefix_proof)
            || !isset($biller->authorization_number_proof)) {
            $result['error'] = true;
            $result['tax_seq'] = 0;
        ...
        if (!$isDateOk) {
            $result['error'] = true;
            $result['tax_seq'] = 0;
            $result['details'] = "Expire Date is already exceed.";
        }
        $result['expire_date'] = $biller->expire_date_proof;
        $result['tax_prefix'] = $biller->prefix_proof;
        $result['tax_security'] = $biller->authorization_number_proof;
        $result['tax_seq_limit'] = $limit_number;
        return $result;
    }
    ...
    public function checkTaxInfoFC($biller, $type)
    {
        ...
        $result['expire_date'] = $biller->expire_date_fc;
        $result['tax_prefix'] = $biller->prefix_fc;
        $result['tax_security'] = $biller->authorization_number_fc;
        $result['tax_seq_limit'] = $limit_number;
        return $result;
    }
    ...
    public function checkTaxInfoCF($biller, $type)
    {
        ...
        if (!isset($biller->sequence_limit_cf) || !isset($biller->expire_date_cf) || !isset($biller->sequence_cf)
            || !isset($biller->prefix_cf) || !isset($biller->authorization_number_cf)) {
            $result['error'] = true;
            $result['tax_seq'] = 0;
            $result['details'] = "Biller Additional info is not available.";
        }
        ...
        $result['expire_date'] = $biller->expire_date_cf;
        $result['tax_prefix'] = $biller->prefix_cf;
        $result['tax_security'] = $biller->authorization_number_cf;
        $result['tax_seq_limit'] = $limit_number;
        return $result;
    }
    ...
    public function checkTaxInfoCN($biller, $type)
    {
        ...
        $result['expire_date'] = $biller->expire_date_cn;
        $result['tax_prefix'] = $biller->prefix_cn;
        $result['tax_security'] = $biller->authorization_number_cn;
        $result['tax_seq_limit'] = $limit_number;
        return $result;
    }
    ...
    public function checkTaxInfoSR($biller, $type)
    {
        ...
        $result['expire_date'] = $biller->expire_date_sr;
        $result['tax_prefix'] = $biller->prefix_sr;
        $result['tax_security'] = $biller->authorization_number_sr;
        $result['tax_seq_limit'] = $limit_number;
        return $result;
    }
    ...
    public function checkTaxInfoGov($biller, $type)
    {
        ...
        $result['expire_date'] = $biller->expire_date_gov;
        $result['tax_prefix'] = $biller->prefix_gov;
        $result['tax_security'] = $biller->authorization_number_gov;
        $result['tax_seq_limit'] = $limit_number;
        return $result;
    }

    public function checkTaxInfoNone($biller, $type)
    {
        //    if($type == "None"){
        $result = array();
        $result['error'] = false;
        $result['tax_prefix'] = null;
        $result['tax_security'] = null;
        $result['tax_seq_limit'] = null;
        $result['tax_seq'] = 0;
        return $result;
    }


    public function dateCompare($exp_date)
    {
        $today = date("Y-m-d");
        $expire = $exp_date; //from database
        ...
        if ($expire_dt < $today_dt) return false;
        return true;
    }
