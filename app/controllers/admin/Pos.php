...
class Pos extends MY_Controller
{
    ...
    public function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, (grand_total+COALESCE(rounding, 0)), paid, (grand_total-paid) as balance, upper(payment_method),sale_status, payment_status, companies.email as cemail")
                ->from('sales')
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, (grand_total+COALESCE(rounding, 0)), paid, (grand_total+rounding-paid) as balance, upper(payment_method),sale_status, payment_status,companies.email as cemail")
                ->from('sales')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->group_by('sales.id');
        }
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $cash_sale = $this->input->post('cash_sale_obj');
            ...
            $tax_type = $this->input->post('tax_type_obj');


            $seq_details = null;
            if ($tax_type == 'None') $seq_details = $this->site->checkTaxInfoNone($biller_details, $tax_type);
            if ($tax_type == 'PROOF') $seq_details = $this->site->checkTaxInfoPROOF($biller_details, $tax_type);
            if ($tax_type == 'FINAL CONSUMER') $seq_details = $this->site->checkTaxInfoFC($biller_details, $tax_type);
            if ($tax_type == 'CREDIT FISCAL') $seq_details = $this->site->checkTaxInfoCF($biller_details, $tax_type);
            if ($tax_type == 'CREDIT NOTE') $seq_details = $this->site->checkTaxInfoCN($biller_details, $tax_type);
            if ($tax_type == 'SPECIAL REGIME') $seq_details = $this->site->checkTaxInfoSR($biller_details, $tax_type);
            if ($tax_type == 'GOVERNMENTAL') $seq_details = $this->site->checkTaxInfoGOV($biller_details, $tax_type);

            if ($seq_details['error'] === true) {
                $this->session->set_flashdata('error', $seq_details['details']);
                redirect($_SERVER["HTTP_REFERER"]);
            }
            ...
            $payment_method = "";
            $payment_status_new = $payment_status;
            ...
            $data = array(
                ...
                'tax_security' => $tax_security,
                'tax_sl' => $tax_sl,
                'tax_seq_limit' => $tax_seq_limit,
                'expire_date' => $tax_expire_date,
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        ...
                        $payment_method = $_POST['paid_by'][$r];
...
//                if ($payment) {
    ...
    public function close_register_new($user_id = NULL)
    {
        $this->sma->checkPermissions('index');
        if (!$this->Owner && !$this->Admin) {
            $user_id = $this->session->userdata('user_id');
...
//        $this->form_validation->set_rules('total_cc_slips', lang("total_cc_slips"), 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = array(
                ...
                'return_amount' => $this->input->post('return_1'),
                'total_sales' => $this->input->post('total_sales_1'),
                'total_sales_credit' => $this->input->post('total_sales_credit_1'),
...
//            admin_redirect("welcome");
            ...
            $this->load->view($this->theme . 'pos/close_register_new', $this->data);
        }
    }
    ...
    public function customer_sales($customer_id = NULL, $warehouse_id = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('pos'), 'page' => lang('pos')), array('link' => '#', 'page' => lang('pos_sales')));
        $meta = array('page_title' => lang('pos_sales'), 'bc' => $bc);
        $this->page_construct('pos/getCustomerSales', $meta, $this->data);
    }


    public function getCustomerSales($customer_id = NULL)
    {
        $this->sma->checkPermissions('index');
    ...
    function register_all_info()
    {
        ...
        $result_all_sales_cash = $this->pos_model->getRegisterAllSalesCash($start_date);
        $result_all_sales_credit = $this->pos_model->getRegisterAllSalesCredit($start_date);
        ...
        $result->result_all_sales_cash = $result_all_sales_cash->total;
        $result->result_all_sales_credit = $result_all_sales_credit->total;
