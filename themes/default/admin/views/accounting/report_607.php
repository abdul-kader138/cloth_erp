...
$(document).ready(function () {
    oTable = $('#SLData').dataTable({
        ...
        'sAjaxSource': '<?=admin_url('accounting/getSales' . ($warehouse_id ? '/' . $warehouse_id : '') .'?v=1'. ($this->input->get('shop') ? '&shop='.$this->input->get('shop') : ''). ($this->input->get('attachment') ? '&attachment='.$this->input->get('attachment') : ''). ($this->input->get('delivery') ? '&delivery='.$this->input->get('delivery') : '')); ?>',
        ...
        "aoColumns": [{"bSortable": false,"mRender": checkbox}, null, null, {"mRender": term_prefix},{"mRender": tax_status}, null, null,null, {"mRender": currencyFormat}, {"mRender": pay_status}],
        ...
        {column_number: 2, filter_default_label: "[<?=lang('Fiscal_Proof_Number');?>]", filter_type: "text", data: []},
...
<?php if ($Owner || $GP['bulk_actions']) {
    echo admin_form_open('accounting/accounting_actions', 'id="action-form"');
}
?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-heart"></i><?=lang('report_607') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
        </h2>
            ...
            <ul class="btn-tasks">
                ...
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=admin_url('accounting/report_607')?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                            foreach ($warehouses as $warehouse) {
                                echo '<li><a href="' . admin_url('accounting/report_607/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
                ...
                <div class="table-responsive">
                    <table id="SLData" class="table table-bordered table-hover table-striped">
                        ...
                        <tr>
                            ...
                            <th><?= lang("date"); ?></th>
                            <th><?= lang("vat_number"); ?></th>
                            <th><?= lang("fiscal_proof"); ?></th>
                            <th><?= lang("biller_type"); ?></th>
                            <th><?= lang("biller"); ?></th>
                            <th><?= lang("customer"); ?></th>
                            <th><?= lang("total_tax_amount"); ?></th>
