    ...
    <div class="container">
        ...
        <div class="header-nav">
            ...
            <ul class="nav navbar-nav pull-right">
                ...
                <li class="dropdown">
                    <a class="btn borange pos-tip" id="close_register" title="<span><?=lang('close_register')?></span>" data-placement="bottom" data-html="true" data-backdrop="static" href="<?=admin_url('pos/close_register_new/'.$this->session->userdata('user_id'))?>" data-toggle="modal" data-target="#myModal">
...
<div id="print">
    ...
    <div id="left-bottom">
        ...
        <input type="hidden" name="cash_sale_obj" value="" id="cash_sale_obj">
        <input type="hidden" name="tax_type_obj" value="" id="tax_type_obj">
...
<div class="form-group">
    <div class="row">
        <div class="col-sm-6">
            <div><span><b><?=lang('c_name');?> :</span>&nbsp;&nbsp;&nbsp;&nbsp;<span id="c_name"></span></b></div>
            <div><span><b><?=lang('credit_status');?> :</span>&nbsp;&nbsp;&nbsp;&nbsp;<span id="credit_status"></span></b></div>
        </div>
    </div>
</div>
<div class="row">
<div class="col-md-10 col-sm-9">
        ...
        <div class="row">
            <div class="col-sm-6">
                <?= form_radio(array(
                    "id"        => "cash_sale",
                    "name"      => "cash_sale"
                ), "1",    set_radio("cash_sale", "1", TRUE)); ?><label
                    for="cash_sale"><?=lang('credit_sale');?></label>&nbsp;&nbsp;&nbsp;&nbsp;
                <?= form_radio(array(
                    "id"        => "cash_sale",
                    "name"      => "cash_sale"
                ), "2",    set_radio("cash_sale", "2", TRUE)); ?><label
                    for="cash_sale"><?=lang('cash_sale');?></label>
            </div>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="well well-sm well_1">
            <div class="payment">
                <div class="row">
                    ...
                    <div class="col-sm-5 col-sm-offset-1" id="payment_mod">
                        <div class="form-group">
    ...
    <button type="button" id="addButton" class="btn btn-primary col-md-12 addButton"><i
...
//        gtotal = formatDecimal(twt);
        ...
        var sales_credit_limit_b=0,total_sales_b=0,total_paid_amount_b= 0, final_credit=0;
        var sales_details=getCreditHistory();
        if(sales_details.credit_limit && sales_details.credit_limit !=null) sales_credit_limit_b= parseFloat(sales_details.credit_limit);
        if(sales_details.sales_amount && sales_details.sales_amount !=null) total_sales_b= parseFloat(sales_details.sales_amount);
        if(sales_details.paid && sales_details.paid !=null) total_paid_amount_b= parseFloat(sales_details.paid);
        var credit_limit_status=((sales_credit_limit_b+total_paid_amount_b) - total_sales_b);
        if(credit_limit_status > 0) final_credit=credit_limit_status;
        if(sales_details) $("#c_name").html(sales_details.customer);
        if(credit_limit_status) $("#credit_status").html(final_credit);
        ...
        <?php }
        ?>
        var tax_val=$('#tax_type').val();
        $('#item_count').text(count - 1);
        $('#paymentModal').appendTo("body").modal('show');
        $('#tax_type_obj').val(tax_val);
    ...
    $(document).on('click', '#submit-sale', function () {
        ...
        if(sales_data.paid) total_paid_amount= parseFloat(sales_data.paid);
        total_sales += parseFloat(gtotal);
        var selectedVal=1;


        var selected = $("input[type='radio'][name='cash_sale']:checked");
        if (selected.length > 0) {
            selectedVal = selected.val();
        }
        if(selectedVal == '1'){
            if ( (sales_credit_limit+total_paid_amount) < total_sales) {
                bootbox.alert('<?=lang('Customer donot have sufficient credit');?>');
                return false;
            }
        }
        // amount check
        if(selectedVal == '1' && total_paid != grand_total){
            bootbox.alert('<?=lang('paid_n_e_payable');?>');
            return false;
        }
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#tax_type').change(function() {
             $('#tax_type_obj').val($(this).val());
        });

        $("input[name='cash_sale']").change(function(){
            var selectedVal=1;
            var selected = $("input[type='radio'][name='cash_sale']:checked");
            if (selected.length > 0) {
                selectedVal = selected.val();
