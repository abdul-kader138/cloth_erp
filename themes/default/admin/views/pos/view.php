...
<div class="text-center">
    ...
    if (!empty($inv->tax_prefix)) {
        echo "<br>";
        if ($pos_settings->cf_value1 != "") {
            echo '<span>' . $pos_settings->cf_title1 . ": " . $pos_settings->cf_value1 . "</span> &nbsp;&nbsp;&nbsp;";
        }
        if ($pos_settings->cf_value2 != "") {
            echo '<span>' . $pos_settings->cf_title2 . ": " . $pos_settings->cf_value2 . "</span> &nbsp;&nbsp;&nbsp;";
        }
    }

    if (!empty($inv->tax_prefix)) {
        if($inv->tax_type=='PROOF') $type=lang("proof");
        if($inv->tax_type=='FINAL CONSUMER') $type=lang("final_consumer");
        if($inv->tax_type=='CREDIT FISCAL') $type=lang("credit_fiscal");
        if($inv->tax_type=='CREDIT NOTE') $type=lang("credit_note");
        if($inv->tax_type=='SPECIAL REGIME') $type=lang("special_regime");
        if($inv->tax_type=='GOVERNMENTAL') $type=lang("governmental");

        echo "<br>";
        echo '<span>' . lang("biller_addition_info") . ": " . $type . "</span> &nbsp;&nbsp;&nbsp;";
    }
...
<?php
$credit_enjoy_days=$customer->credit_enjoy_days;
$invoice_dates=date('d-m-Y', strtotime($inv->date. ' + '.$credit_enjoy_days .' days'));
$expire_dates=date('d-m-Y', strtotime($inv->expire_date));
...
if (!empty($inv->tax_prefix)) {
    echo "<span class='left'>" .lang("additional_sl") . ": " . $inv->tax_sl . "</span>";
    echo "<span class='right'>" .lang("authorization_number") . ": " . $inv->tax_security . "</span>";
}
echo "<span class='left'>" .lang("bill_term") . ": " . ($inv->payment_method == 'credit' ? strtoupper(lang("credit")) : strtoupper(lang("cash"))) . "</span>";
echo "<span class='right'>" .lang("client_phone") . ": " . $customer->phone . "</span>";
if($inv->expire_date) {
    echo "<span class='left'>" . lang("expire_date") . ": " . $expire_dates . "</span>";
}
if($customer->credit_enjoy_days && $inv->payment_method == 'credit') {
    echo "<span class='right'>" .lang("invoice_expire_date") . ": " . $invoice_dates . "</span>";
}
if (!empty($inv->tax_prefix)) {
    echo "<span class='left'>" . lang("vat_number") . ": " . $customer->vat_no . "</span>";
}
//echo "<span class='right'>" .lang("invoice_expire_date") . ": " . $invoice_dates . "</span>";
...
<table class="table table-condensed" cellspacing="0"  border-spacing="0" border="0">
    <thead>
    <tr>
        <th style="text-align:left; width:210px;" ><?php echo lang("product") ?></th>
        <th style="text-align:left; width:60px;" ><?php echo lang("rate") ?></th>
        <th style="text-align:left; width:60px;" ><?php echo lang("tax") ?></th>
        <th style="text-align:right; width:80px;" ><?php echo lang("total") ?></th>
    </tr>
    </thead>
    <tbody>
    <?php
    ...
    if ($return_rows) {
        ...
        foreach ($return_rows as $row) {
            ...
            echo '<tr><td style="text-align:left;  width:5px;"> #' . $r . '</td><td style="text-align:left; width:210px;">' . $row->product_name. '(' . $row->product_code . ')'.'(' . $this->sma->formatQuantity($row->quantity)  . ')</td>';
            echo '<td style="text-align:left; width:60px;" class="no-border border-bottom">'. $this->sma->formatMoney($row->net_unit_price).'</td><td style="text-align:left; width:80px;" class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
            $r++;
        }
    }
    ...
    <tr>
        <th><?=lang("Product Tax");?>(<?= $default_currency->code; ?>)</th>
